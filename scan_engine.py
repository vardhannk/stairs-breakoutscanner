...
FIELDS: dict[str, Field] = {
    ...
    "is_fund": Field("is_fund", "bool", "ETF / Fund",
                     group="Miscellaneous",
                     help="An exchange-traded fund, not a company. On NSE these "
                          "list in series EQ exactly like a share, so nothing "
                          "in the instrument dump distinguishes them. They are "
                          "flagged, never removed, and they carry no RS Rating "
                          "because a percentile against companies is not a "
                          "meaningful statement about gold or silver."),
    ...
    "approx_listing_days": Field("approx_listing_days", "int", "Trading Days Listed",
                                 help="Bars available. The honest measure of "
                                      "how much history exists — RS Rating "
                                      "needs 252, Minervini needs 200."),
    "pct_of_listing_range": Field("pct_of_listing_range", "num",
                                  "% of Listing Range", "%",
                                  help="Where price sits between its lowest "
                                       "and highest since listing. The "
                                       "stand-in for '% of 52w high' when "
                                       "there has not been a 52-week year."),

    # ── breakout detail, as the old scan's table showed it ─────────────
    # The detector returns all of these; the snapshot used to discard them,
    # which is why the table view lost Mode, Prior High/Low, Bar Date,
    # Lookback and 52W High. They describe the HEADLINE timeframe — daily if
    # it fired, otherwise the first that did.
    "breakout_timeframe": Field("breakout_timeframe", "text", "Timeframe",
                                help="Which timeframe the headline figures "
                                     "describe. breakout_timeframes lists "
                                     "every one that fired."),
    "breakout_mode": Field("breakout_mode", "text", "Mode",
                           help="standard | strict"),
    "breakout_prior_high": Field("breakout_prior_high", "num", "Prior High", "₹"),
    "breakout_prior_low": Field("breakout_prior_low", "num", "Prior Low", "₹"),
    "breakout_bar_time": Field("breakout_bar_time", "text", "Bar Date",
                               help="The bar the breakout was detected on. On "
                                    "weekly and monthly this is the period "
                                    "END, so it can read as a future date."),
    "breakout_lookback": Field("breakout_lookback", "num", "Lookback", "bars"),
    "breakout_is_52w_high": Field("breakout_is_52w_high", "bool", "52W High?"),
    "breakout_vol_ratio": Field("breakout_vol_ratio", "num", "Vol Ratio", "×",
                                help="From the detector, on the breakout bar. "
                                     "volume_ratio is the pattern module's "
                                     "own measure and may differ."),

    # ── index membership: scan once, filter many ───────────────────────
    # Every NSE index is a subset of the universe this snapshot already
    # covers, so an index view is a WHERE clause rather than a second scan.
    "cap_band": Field("cap_band", "text", "Cap Band", group="Miscellaneous",
                      help="Large (Nifty 100) | Mid (Midcap 150) | Small "
                           "(Smallcap 250) | Micro (Microcap 250) | Below "
                           "Microcap. NSE's own classification — complete, "
                           "unlike market_cap_cr which the data source "
                           "never supplies."),
    "in_nifty100": Field("in_nifty100", "bool", "In NIFTY 100",
                         group="Miscellaneous"),
    "in_midcap150": Field("in_midcap150", "bool", "In Midcap 150",
                          group="Miscellaneous"),
    "in_smallcap250": Field("in_smallcap250", "bool", "In Smallcap 250",
                            group="Miscellaneous"),
    "in_microcap250": Field("in_microcap250", "bool", "In Microcap 250",
                            group="Miscellaneous"),
    "in_totalmarket": Field("in_totalmarket", "bool", "In Total Market",
                            group="Miscellaneous"),
    "in_fno": Field("in_fno", "bool", "In F&O",
                    group="Miscellaneous",
                    help="Has stock futures on NSE. Derived from the Kite "
                         "instrument dump each week, not hardcoded — SEBI "
                         "revises the list and a frozen copy goes wrong "
                         "silently. Index futures are excluded."),

    # ── freshness ──────────────────────────────────────────────────────
    # Every snapshot row is stamped with the BUILD date, but an illiquid
    # stock may not have traded that day. On the full NSE universe 326 of
    # 3,046 symbols had no bar for the build date — they simply did not
    # print. Without this a stock that broke out on Wednesday and has not
    # traded since still appears under "broke out today".
    "last_bar_date": Field("last_bar_date", "text", "Last Traded Bar",
                           help="Date of the most recent bar for this symbol. "
                                "Older than the snapshot date means it did "
                                "not trade that day."),
    "traded_today": Field("traded_today", "bool", "Traded On Snapshot Date",
                          help="False for stocks that did not print a bar on "
                               "the build date. Any 'today' screen should "
                               "require this."),
...
PREDEFINED: dict[str, dict] = {
    ...
    "★ Daily Screen — breakout in a strong stock": {"logic": "AND", "rules": [
        ...
        # It has to have traded on the day the snapshot claims to describe.
        # 326 of 3,046 symbols had no bar for the build date, so without this
        # a stock that broke out two days ago and has not printed since is
        # presented as breaking out today.
        {"field": "traded_today", "op": "is_true"},
        {"field": "rs_rating", "op": ">=", "value": 80},
    ...
    # Everything young enough that RS Rating cannot exist, breakout or not.
    # This is the IPO page's universe: no trigger required, because the
    # question there is "what has listed recently and is behaving well",
    # not "what fired today".
    "★ Recent Listings — all": {"logic": "AND", "rules": [
        {"field": "rs_rating", "op": "is_null"},
        {"field": "turnover_30d_cr", "op": ">=", "value": 1},
    ]},
