...
def get_market_regime_classification(lookback_days: int = 365) -> dict:
    ...
    try:
        ...
        # -- label each cluster from its own centroid ------------------------
        # This used to fall back to forcing a one-of-each split by momentum
        # rank whenever the threshold rules did not yield three distinct
        # names. With a +/-1% band around the 50-day EMA the middle bucket is
        # usually empty, so that fallback ran on most days, and it labelled
        # the lowest-momentum cluster "Volatile Bearish" whatever its momentum
        # actually was. In a sustained advance the weakest of three positive
        # clusters was reported as a weak and volatile market.
        #
        # Duplicate names are fine. Only the latest day's cluster is read
        # below, and if all three centroids sit above the EMA then "above
        # trend" is the honest answer, not a defect to be corrected.
        #
        # Rule order is unchanged from the original: momentum is tested first,
        # so a strongly trending but volatile cluster stays "Bullish Trend".
        vol_mean = float(profiles["volatility"].mean())
        cluster_names = {}
        for c in profiles.index:
            mom = float(profiles.loc[c, "momentum"])
            vol = float(profiles.loc[c, "volatility"])
            if mom > 0.01:
                cluster_names[c] = ("Bullish Trend", "#3dd68c")
            elif mom < -0.01 or vol > vol_mean * 1.1:
                cluster_names[c] = ("Volatile Bearish", "#ff6b6b")
            else:
                cluster_names[c] = ("Neutral/Choppy", "#ffc107")

        # Latest day's cluster
        latest_feat = df_feats.iloc[-1]
        latest_cluster = int(latest_feat["cluster"])
        name, color = cluster_names[latest_cluster]

        # Describe what was measured. The wording this replaces claimed breakout
        # success rates and false-breakout risk; nothing in this codebase
        # measures breakout follow-through, so those were assertions with no
        # evidence behind them. This function also reads ^NSEI only -- it never
        # touches the ~3,000 stocks the screener ranks.
        #
        # Both numbers are shown deliberately. The NAME comes from the cluster's
        # centroid, the returned "momentum"/"volatility" fields are the latest
        # bar, and the two can disagree -- today can sit at -0.9% inside a
        # cluster averaging -4%. Showing only one of them is what would make the
        # banner look wrong.
        _cm = float(profiles.loc[latest_cluster, "momentum"])
        _lm = float(latest_feat["momentum"])
        _lv = float(latest_feat["volatility"])
        desc = ("This regime averages %+.1f%% vs the 50-day EMA; today %+.1f%%. "
                "20-day realised volatility %.0f%%, against a 2-year mean of "
                "%.0f%%. Index only — not a measurement of breakout outcomes."
                % (_cm * 100.0, _lm * 100.0, _lv * 100.0, vol_mean * 100.0))
